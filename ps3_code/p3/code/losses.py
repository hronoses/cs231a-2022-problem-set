...
def depth_loss(y_true, y_pred, theta=0.1, device="cuda"):
    ...
    l_edges = torch.mean(torch.abs(dy_pred - dy_true) + torch.abs(dx_pred - dx_true), 1)
    # TODO use torch to calculate the mean absolute difference between the true and predicted gradients
